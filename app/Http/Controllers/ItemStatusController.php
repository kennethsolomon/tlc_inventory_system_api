...
class ItemStatusController extends Controller
{
    ...
    public function updateOrCreateItemStatus(ItemStatusPostRequest $request)
    {
        try {
            ...
            $log = ['user' => auth()->user()->email, 'action' => 'Create/Update', 'description' => 'Item Status  with ID ' . $item_status->id . ' has been created/updated.'];
            Log::create($log);
            DB::commit();
    ...
    public function destroy(ItemStatus $item_status)
    {
        try {
            ...
            $log = ['user' => auth()->user()->email, 'action' => 'Delete', 'description' => 'Item Status with ' . $item_status->id . ' has been deleted.'];
            Log::create($log);
